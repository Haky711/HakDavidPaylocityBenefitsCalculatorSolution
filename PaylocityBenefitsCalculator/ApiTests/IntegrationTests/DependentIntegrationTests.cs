...
public class DependentIntegrationTests : IntegrationTest
{
    ...
    [Fact]
    public async Task WhenAddingAnotherDependentSpouse_ShouldReturn406()
    {
        var response = await HttpClient.PostAsJsonAsync("/api/v1/dependents", new PostDependentDto
        {
            FirstName = "Spouse2",
            LastName = "Morant",
            Relationship = Relationship.Spouse,
        ...
        await response.ShouldReturn(HttpStatusCode.NotAcceptable);
    }
}
